...
export default function ExportPage() {
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-10 font-serif text-gray-900 print-page">
      ...
      <header className="mb-8 border-b border-gray-300 pb-6">
        ...
        <div className="mt-4 grid grid-cols-2 gap-x-8 gap-y-2 text-sm text-gray-700">
          ...
          {sequence.transitionMinutes !== undefined && (
            <div className="flex gap-2 col-span-2">
              <span className="font-semibold w-20 shrink-0">Timing:</span>
              <span>{sequence.totalHoldMinutes} min hold + {sequence.transitionMinutes} min transitions ≈ {sequence.totalSessionMinutes} min</span>
            </div>
          )}
        </div>
      </header>
      ...
      {sequence.sutra?.text && (
        <blockquote className="mb-8 border-l-4 border-gray-300 pl-4">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Yoga Sutra</p>
          <p className="italic text-gray-800">&ldquo;{sequence.sutra.text}&rdquo;</p>
          <footer className="mt-1 text-sm text-gray-600">- {sequence.sutra.attribution}</footer>
        </blockquote>
      )}
              ...
              {sequence.items.map((item, idx) => (
                <tr key={`${item.pose.slug}-${idx}`} className="align-top pose-row">
                  ...
                  <td className="pose-table-cell">
                    <span className="font-medium">{resolveDisplayName(item.pose, ctx.style)}</span>
                    {item.pose.sanskrit && (
      ...
      <footer className="mt-12 pt-4 border-t border-gray-200 text-xs text-gray-400 text-center print-footer">
        Generated by YogaKit, a free yoga sequencing tool
      </footer>
    </main>
  )
}
